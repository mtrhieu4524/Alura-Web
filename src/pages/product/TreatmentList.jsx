...
function TreatmentListPage() {
    ...
    useEffect(() => {
        document.title = 'Alurà - Medicals & Treatments';
    }, []);
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                if (j.success && Array.isArray(j.data)) setBrandOptions(j.data);
            } catch (err) {
        ...
        (async () => {
            try {
                const res = await fetch(`${import.meta.env.VITE_API_URL}/product-types`);
                const j = await res.json();
                if (Array.isArray(j)) {
                    const opts = j
                        .filter(pt => pt.category?._id === COSMETICS_CATEGORY_ID)
        ...
        const fetchProducts = async () => {
            try {
                const response = await fetch(
                    `${import.meta.env.VITE_API_URL}/products?pageIndex=1&pageSize=100&categoryId=685f755db792e430e6925db0`
                );
    ...
    const filterConfigs = [
        {
            label: "Type", state: type, setter: setType, options: typeOptions
        },
        {
    ...
    return (
        <div className="ProductList">
            <Breadcrumb items={[{ name: 'Home', link: '/' }, { name: 'Medical & Treatment' }]} />
